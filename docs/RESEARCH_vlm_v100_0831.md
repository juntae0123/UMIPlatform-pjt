...
## 2-bis. GPU 가 10대다 — 무엇이 바뀌고 무엇이 안 바뀌나 (2026-09-01 추가)

**Tesla V100 32GB × 10대 = 320GB** 로 확정됐다. 먼저 오해를 지운다.

| 바뀌지 않는 것 | 이유 |
|---|---|
| bf16 불가 · FA2 불가 · LLM.int8() 불가 · vLLM 미지원 | **장당 제약이다.** 10장이어도 각 장은 여전히 sm_70 |
| 한 모델의 학습 속도 | 모델을 쪼개지 않는 한 그대로 |
| **모델 크기 상한** | **Jetson Orin Nano 8GB 가 정한다 (~3B).** GPU 메모리가 아니다 |
| 데이터 수집 시간 | 사람 손이 하는 일이라 병렬화가 안 된다 |

| 새로 가능해진 것 | 내용 |
|---|---|
| **스킬 5종 동시 학습** | 우리 정책은 1,307,974 파라미터라 DDP 는 무의미. **GPU 당 독립 잡**이 맞다 |
| **스윕 병렬** | 같은 데이터셋 × 시드·하이퍼파라미터 여러 개 → **데이터 1벌로 반복을 여러 번** 도는 효과 |
| VLM + 정책 동시 | 한 장에서 LoRA, 나머지로 정책 |
| 큰 VLM 학습 | **의미 없다.** Jetson 8GB 가 상한이라 3B 넘게 학습해도 배포를 못 한다 |

**결론: 320GB 의 값어치는 "큰 모델"이 아니라 "많은 실험"이다.**
그리고 학습이 병렬화되면서 **병목은 전부 수집으로 옮겨간다** — 4주 일정을 결정하는 숫자가
GPU 시간에서 **UMI 수용률**로 바뀌었다 (LIMITS L28).

미측정 항목: 노드 구성·인터커넥트(M7), 10잡 동시 실행 시 처리량 저하(M8), DDP 스케일링(M9).
