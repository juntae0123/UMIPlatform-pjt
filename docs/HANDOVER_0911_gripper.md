...
### ✅ S1. 완료 (2026-09-11) — `MEASURE_exec_envelope_0911.md`

도구는 `tools/probe_exec_envelope.py` + `eval/exec_limits.py` 로 만들어졌다.
**`check_policy_real_limits.py` 를 새로 만들지 마라. 판정 로직은 `eval/exec_limits.py`
하나뿐이고 거기서 import 한다.**

| 타깃 | A 롤아웃 | B 클램프 | C 폐쇄시각 상한 | ROS2 | direct |
|---|---:|---:|---:|---:|---:|
| scripted | 84/100 | 84 (±0) | 84 (±0) | **100%** | 0% |
| dagger binary seed0 | 6 | 6 (±0) | 11 (+5) | **98%** | 0% |
| dagger binary seed1 | 69 | 69 (±0) | 72 (+3) | **98%** | 0% |
| dagger binary seed2 | 25 | 25 (±0) | 39 (+14) | **84%** | 0% |

**결론 셋**

1. **실행 경로는 ROS2 다.** direct-motor 는 시간 스케일링 x20(=1.5Hz)에서야 통과한다 🔵.
   30Hz 정책 궤적에 못 쓴다 (`waypoint.duration` 100% 위반, `peak_accel` 최대 152.6배)
2. **ROS2 위반은 URDF 소수점 절삭 산물이다** 🟢. `max_ratio` 가 네 타깃 모두 정확히
   `1.0000x`. wrist_flex 상한 URDF `1.65806` vs 우리 `1.6580627293335335` (2.73e-6 rad),
   shoulder_pan 하한 2.18e-6 rad. 클램프하면 성공률 ±0.0%p → **무해하다.**
   처방: 우리 쪽 마이크로 마진 클램프 + ROS·HW 에 URDF 자릿수 정합 요청 (L73)
3. **`elbow_flex` 하한 위반은 0건이었다.** 하루 종일 "S1 의 첫 숫자"라고 했던 것이
   실제로는 0이다. config 불일치는 실재하지만 **궤적이 그 영역을 안 쓴다**
4. **폐쇄 시각 예측에 투자할 값이 작다** — 완벽히 맞혀도 +3~14%p. 보조 판정 6번 기준
   미달. **S3 의 우선순위를 내린다.** 남은 병목은 팔 정밀도와 학습 불안정이다

⚠️ **ROS2 통과율은 하한이다** (L74). `JointTrajectoryController` 의 30Hz 보간점을
재현하지 않았고, 위반이 정확히 경계에서 나므로 보간 overshoot 는 더 걸릴 수 있다.
클램프에 마진을 조금(예: 1e-4 rad) 주는 게 안전하고 비용은 실측상 0이다.

### (참고) S1 원안 — 이미 끝났다. 아래는 이력이다
...
### S3. 폐쇄 시각 상한 — **우선순위 내림** (dagger 기준 답이 나왔다)

`probe_exec_envelope` 의 패스 C 가 이걸 공짜로 답했다: dagger 에서 +3 / +5 / +14%p.
보조 판정 6번(+15%p 이상이면 투자 가치)에 미달이다. **폐쇄 시각은 이미 거의 최적이고,
닫는 순간 4.9mm 가 scripted 4.7mm 수준인 것과 부합한다.**

`PREREG_oracle_best_tick_0911.md` 는 **v5 대상**이라 아직 열려 있다 (v5 는 닫는 순간이
25.6mm 라 여지가 크다). 다만 우리가 미는 계열은 dagger 이므로 **가치가 낮다.**
...
### S4. dagger 범위 63%p 의 원인 ★ **지금 여기다** (seed0 6/100 · 미폐쇄 67)

S1 이 실행 계층을 닫았으므로 **남은 병목은 여기다.**

**진행 중 (2026-09-11):** `tools/probe_gripper_logits.py` 작성 완료,
사전등록 `docs/PREREG_gripper_logits_0911.md`. 미폐쇄가 **눌림**(p 가 내내 0.5 아래)인지
**요동**(넘되 유지 안 됨)인지 가른다 — 처방이 정반대다. 임계값 스윕 {0.3~0.7} 포함이고
**0.5 가 현행과 같아야 하므로 그 자체가 계측기 검증**이다 (`--expected-at-half 6 69 25`).
